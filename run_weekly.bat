@echo off
REM ============================================================
REM  run_weekly.bat - BYOCORE 주간 리포트 자동 실행 (Windows 작업 스케줄러용)
REM    python -m src.reporter weekly  (직전 7일 매출 집계 -> 카카오 나에게 보내기)
REM    실행 로그: logs\weekly_YYYYMMDD.log  (성공/에러 기록)
REM ============================================================
setlocal
...
set "LOGFILE=logs\weekly_%TODAY%.log"

REM --- 실행 헤더 기록 ---
echo.>>"%LOGFILE%"
echo ============================================================>>"%LOGFILE%"
echo [%date% %time%] run_weekly.bat 시작>>"%LOGFILE%"
echo ------------------------------------------------------------>>"%LOGFILE%"

REM --- 리포터 실행: stdout+stderr 모두 로그에 append ---
"C:\Users\Administrator\AppData\Local\Programs\Python\Python313\python.exe" -m src.reporter weekly >>"%LOGFILE%" 2>&1
set "RC=%ERRORLEVEL%"
