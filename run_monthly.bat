@echo off
REM ============================================================
REM  run_monthly.bat - BYOCORE 월간 리포트 자동 실행 (Windows 작업 스케줄러용)
REM    python -m src.reporter monthly  (직전 30일 매출 집계 -> 카카오 나에게 보내기)
REM    실행 로그: logs\monthly_YYYYMMDD.log  (성공/에러 기록)
REM ============================================================
setlocal
...
set "LOGFILE=logs\monthly_%TODAY%.log"

REM --- 실행 헤더 기록 ---
echo.>>"%LOGFILE%"
echo ============================================================>>"%LOGFILE%"
echo [%date% %time%] run_monthly.bat 시작>>"%LOGFILE%"
echo ------------------------------------------------------------>>"%LOGFILE%"

REM --- 리포터 실행: stdout+stderr 모두 로그에 append ---
"C:\Users\Administrator\AppData\Local\Programs\Python\Python313\python.exe" -m src.reporter monthly >>"%LOGFILE%" 2>&1
set "RC=%ERRORLEVEL%"
